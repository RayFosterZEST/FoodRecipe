    ...
    useEffect(() => {
      const fetchrecipes = async () => {
            ...
            try{
                ...
                if(recipeList.length>0)setrecipes(recipeList);
            }catch(error){
                console.log('Recpies not found');
            }
            setLoading(false);
        };
                ...
                <View key={index} style={styles.recipeCard} testID="recipeCard">
                  <TouchableOpacity testID="handlerecipeBtn" onPress={() => handlerecipeClick(recipe)}>
                    ...
                    <Text style={styles.recipeDescription} testID="recipeDescp">
                        {
                            recipe.description.length>50?`${recipe.description.slice(0,47)}...`:recipe.description
                        }
                    </Text>
                  </TouchableOpacity>
